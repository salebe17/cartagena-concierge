...
export default function TechnicianBidPage() {
    ...
    return (
        <div className="min-h-screen p-6 max-w-2xl mx-auto pt-24">
            <Link href="/technician/dashboard" className="inline-flex items-center text-[var(--color-text-secondary)] hover:text-white transition-colors mb-8">
            ...
            >
                <div className="p-8 border-b border-[rgba(255,255,255,0.05)] bg-[rgba(0,0,0,0.2)]">
                    ...
                    <h1 className="text-2xl sm:text-3xl font-bold text-white mb-4 leading-tight">{request.description}</h1>
                ...
                <div className="p-8">
                    <div className="mb-8">
                        <p className="text-xs text-[var(--color-text-secondary)] uppercase tracking-wider mb-1">Client is offering</p>
                        <p className="text-4xl font-extrabold text-white">${request.offered_price || request.quoted_price || 0} COP</p>
                    </div>
                    ...
                    {hasBid ? (
                        <div className="bg-[rgba(0,229,255,0.1)] border border-[var(--color-primary)] p-6 rounded-2xl flex flex-col items-center text-center">
                            ...
                            <p className="text-[var(--color-text-secondary)]">
                                You have offered to do this job. Waiting for the client to review your bid and accept it.
                            </p>
                        </div>
                    ) : request.status !== 'pending' ? (
                        <div className="bg-red-500/10 border border-red-500/30 p-6 rounded-2xl flex flex-col items-center text-center">
                            <h3 className="text-xl font-bold text-white mb-2">Job is Closed</h3>
                            <p className="text-[var(--color-text-secondary)]">
                        ...
                        <form onSubmit={handlePlaceBid} className="space-y-6">
                            <div className="space-y-3">
                                ...
                                <div className="relative">
                                    <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--color-text-secondary)] text-xl">$</span>
                                    <input
                                        type="number"
                                ...
                                <div className="flex gap-2">
                                    {[request.offered_price, request.offered_price * 1.2, request.offered_price * 1.5].map((preset) => (
                                        <button
                                            key={preset}
                                            type="button"
                            ...
                            <div className="pt-4 flex gap-4">
                                ...
                                >
                                    {submitting ? <Loader2 className="w-6 h-6 animate-spin" /> : "Place Custom Bid"}
                                </button>
                            </div>
                        </form>
                    )}
